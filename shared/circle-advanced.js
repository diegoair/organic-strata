  ...
  function run(paper, R, o) {
    ...
    if (o.trim === 'cut') {
      ...
      for (let i = 0; i < count; i++) {
        ...
        g.rotate(i / count * 360 - 90, new paper.Point(0, 0));
        g.translate(C);
    ...
    return shape ? shape.pathData.replace(/-?\d+\.\d+/g, m => String(+(+m).toFixed(3))) : '';
  }

  Organica.circleAdvanced = { build, isActive, DEFAULTS };
})(window);
