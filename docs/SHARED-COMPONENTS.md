...
| `Organica.zip` (store-only writer) | **Candidate — built then removed `2026-09-01`.** `shared/zip.js` shipped for Mote's SVG-sequence export, deleted the same day when that feature was dropped (382 MB / 3 s, renderers hang on 17 MB-per-frame SVG). The ~60-line store-only (method 0) writer + CRC32 lives in git history (`shared/zip.js` at commit `cc532d6`). Rebuild it if a real multi-file export lands — **Colornet's "Batch"** (currently sequential individual downloads with a 220–350 ms anti-burst delay) is the standing candidate. | not built |
