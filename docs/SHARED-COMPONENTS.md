...
| `Organica.zip` | **Component shipped** `2026-09-01` — `shared/zip.js`. `Organica.zip()` → `{ add(name, bytes /* Uint8Array */), blob() }`. Minimal **STORE-only** (method 0, no compression) writer + a lazy CRC32 table; ~60 lines, no dependency. Local file headers (`0x04034b50`) + central directory (`0x02014b50`) + EOCD (`0x06054b50`); DOS date/time fixed at 1980-01-01; UTF-8 names (bit 11). Intended for bundling **already-compressed** payloads — Mote gzips each SVG frame (`CompressionStream`) to `.svgz` then zips the set. **First consumer:** Mote's SVG-sequence export. Candidates for adoption: Colornet's "Batch" (currently sequential individual downloads with a 220–350 ms anti-burst delay), any future Loom multi-page / plate export. If real compression is ever needed, add a deflate method rather than a second lib. | first consumer shipped |
