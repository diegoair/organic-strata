...
| **`.upload-btn` / `.org-file-input`** | `.upload-btn` in `organica-panel.css` (Camo Turing / Soul adopted exact; Pollen / Membrane / **Spore** keep a 1-line delta — Spore's `.upload-mark-btn` renamed to `.upload-btn` 2026-08-30). `.org-file-input` on **7 tools / 15 inputs** (komorebi/halide/spore/pollen/soul/colornet/membrane). Still open: camo-turing ×5, fvs ×2, genesis ×1 inline sr-only `<input type=file>` blocks; Genesis's own `--fs-xs`-scale `.upload-btn` left local. | — |
